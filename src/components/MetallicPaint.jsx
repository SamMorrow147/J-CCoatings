...
void main() {
    ...
    // Apply the logo mask - img is already sampled above
    float mask = 1.0 - img.r; // Invert the mask (white background becomes transparent)
    
    if (mask > 0.5) {
        // Inside the logo shape - show enhanced metallic effect
        // Make the colors more vibrant and visible
        vec3 enhancedColor = color;
        enhancedColor = mix(enhancedColor, vec3(0.9, 0.8, 0.6), 0.3); // Add golden tint
        enhancedColor *= 1.5; // Increase brightness
        fragColor = vec4(enhancedColor, 1.0);
    } else {
        // Outside the logo shape - transparent
...
export default function MetallicPaint({ imageData, params = defaultParams }) {
  ...
  function updateUniforms() {
    ...
    console.log('MetallicPaint: Updating uniforms with params:', params);
    ...
    console.log('MetallicPaint: Uniforms updated');
  }

  useEffect(() => {
    function initShader() {
      const canvas = canvasRef.current;
      console.log('MetallicPaint: Initializing canvas:', canvas);
      ...
      console.log('MetallicPaint: WebGL2 context:', gl);
      ...
      console.log('MetallicPaint: Shaders compiled and linked successfully');
  ...
  useEffect(() => {
    ...
    function render(currentTime) {
      ...
      if (Math.floor(currentTime / 1000) !== Math.floor((currentTime - deltaTime) / 1000)) {
        console.log('MetallicPaint: Rendering frame at', Math.floor(currentTime / 1000), 's');
      }
  ...
  useEffect(() => {
    ...
    function resizeCanvas() {
      ...
      console.log('MetallicPaint: Resizing canvas to', size, 'x', size);
      
      canvasEl.width = size * devicePixelRatio;
      canvasEl.height = size * devicePixelRatio;
      ...
      console.log('MetallicPaint: Canvas dimensions:', canvasEl.width, 'x', canvasEl.height);
      console.log('MetallicPaint: Viewport:', gl.getParameter(gl.VIEWPORT));
    }
  ...
  useEffect(() => {
    ...
    try {
      console.log('MetallicPaint: Uploading texture:', imageData?.width, 'x', imageData?.height);
